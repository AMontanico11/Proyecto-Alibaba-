...
export function validatos(aUsuario, aContrasena){
  var ListUsuarios = Lista_user();
  var bAcceso =false;

  for(var i = 0; i < ListUsuarios.length; i++){
    if(aUsuario == ListUsuarios[i][1] && aContrasena == ListUsuarios[i][2]){
      bAcceso = true;

      sessionStorage.setItem('usuarioActivo', ListUsuarios [i][2] + ' ' + ListUsuarios[i][2]);
      sessionStorage.setItem('PerfilUsuarios', ListUsuarios[i][0]);
    }
  }
  return bAcceso;
